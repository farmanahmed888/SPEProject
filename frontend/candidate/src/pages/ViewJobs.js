...
function ViewJobs() {
  ...
  const handleViewClick = async (id, company, candidateId) => {
    try {
      ...
      await axios.post("http://loadbalancer:8000/candidate/apply", dto);
  ...
  const fetchAppliedJobs = async () => {
    try {
      const appliedJobsResponse = await axios.get(
        `http://loadbalancer:8000/candidate/appliedJobs/${localStorage.getItem(
          "candidateId"
        )}`
      );
    ...
    const fetchData = async () => {
      try {
        ...
        const allJobsResponse = await axios.get(
          "http://loadbalancer:8000/interviewer/all-active-jobs"
        );
